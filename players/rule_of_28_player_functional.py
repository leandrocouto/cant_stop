...
class Rule_of_28_Player_2(Player):
...
#                 env = self.init_env()
#                 score1 = self.program_yes_no.interpret(env)
                # Calculate score
                score = np.sum(
                                list(
                                    map(
                                        lambda col : (self.number_advanced_this_round(col) + 1) * self.progress_value[col], neutrals
                                        )
                                    )
                                )
                # Difficulty score
