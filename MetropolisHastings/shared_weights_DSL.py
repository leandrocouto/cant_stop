class SharedWeightsDSL:
    ...
    def __init__(self, start):
        ...
        self._grammar['function_str_score'] = [# Strictly "string" actions
                                'SharedWeightsDSL.get_player_score(state)',
                                'SharedWeightsDSL.get_opponent_score(state)',
                                'SharedWeightsDSL.number_cells_advanced_this_round(state)',
                                'SharedWeightsDSL.get_weight_of_neutral_columns(state, weights)'
                                ]
        self._grammar['function_num_score'] = [# Strictly "numeric" actions
                                'SharedWeightsDSL.advance_in_action_col(actions[i])',
                                'SharedWeightsDSL.does_action_place_new_neutral(actions[i], state)',
                                'SharedWeightsDSL.get_weight_for_action_columns(actions[i], weights)'
                                ]
        self._grammar['OP'] = ['+', '-', '*']
    ...
    def rule_of_28(state):
        ...
        for column in range(state.column_range[0], state.column_range[1]+1):
            if column not in neutral:
                ...
                advance = SharedWeightsDSL.number_cells_advanced_this_round_for_col(state, column)
    ...
    def number_cells_advanced_this_round_for_action(state, action):
        ...
        advancement = 0
        for a in action:
            advancement += SharedWeightsDSL.number_cells_advanced_this_round_for_col(state, a)
            advancement += 1
        return advancement
    @staticmethod
    ...
    def number_cells_advanced_this_round(state):
        ...
        counter = 0
        for column in range(state.column_range[0], state.column_range[1]+1):
            counter += SharedWeightsDSL.number_cells_advanced_this_round_for_col(state, column)
        return counter
