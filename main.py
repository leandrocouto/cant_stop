...
def main():
    victory_1 = 0
    victory_2 = 0
    config = Config(c =1, n_simulations = 10, n_games = 100, n_players = 2, 
                    dice_number = 4, dice_value = 3, column_range = [2,6], 
                    offset = 2, initial_height = 1, mini_batch = 20, 
                    sample_size = 100, n_games_evaluate= 50, victory_rate = .55,
                    alphazero_iterations = 1)
    ...
    output_prob_dist = Dense(27, activation='softmax')(hidden_fc_prob_dist_2)
    #Value of a state
    hidden_fc_value_1 = Dense(100, activation='relu')(flat)
    hidden_fc_value_2 = Dense(100, activation='relu')(hidden_fc_value_1)
    output_value = Dense(1, activation='tanh')(hidden_fc_value_2)
    ...
    for _ in range(config.alphazero_iterations):
        ...
        # Training loop
        #
        mini_batch_instances = random.sample(dataset_for_network, 
                                                k = config.mini_batch)
